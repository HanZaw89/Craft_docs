...
function setRadialOffset(circleEl, percentEl, percentage) {
  if (!circleEl) return;
...
function updateAllProgressMetrics() {
  ...
  setRadialOffset(elements.yearProgressCircle, elements.yearProgressPercent, yearPercent);
  ...
  setRadialOffset(elements.monthProgressCircle, elements.monthProgressPercent, monthPercent);

  // 3. Week Progress
  // Get Sunday at 00:00:00 of this week
  ...
  setRadialOffset(elements.weekProgressCircle, elements.weekProgressPercent, weekPercent);
  ...
  setRadialOffset(elements.dayProgressCircle, elements.dayProgressPercent, dayPercent);
}
...
function renderTimelineView() {
  ...
  if (sortedTasks.length === 0) {
    container.innerHTML = `
      <div class="empty-state">
        <i data-lucide="milestone" class="empty-icon"></i>
  ...
  sortedTasks.forEach((task, index) => {
    ...
    // Add staggered entry animations via inline style
    const delay = index * 0.08;
    
    html += `
      <div class="timeline-item ${statusClass}" style="animation-delay: ${delay}s" onclick="openEditModal('${task.id}')">
        <div class="timeline-node-wrapper">
          <div class="timeline-node" title="Click to complete goal" onclick="event.stopPropagation(); toggleTaskCompleted('${task.id}')">
...
async function shiftTaskStatus(id, newStatus) {
  ...
  await apiUpdateTask(id, task.title, properties, true); // suppressReload = true for fast visual response
}
...
async function toggleTaskCompleted(id) {
  ...
  // Re-sync with full fetch to refresh all states
  fetchTasks();
}
...
function renderSidebarWidgets() {
  ...
  // Find incomplete goals with due dates or action dates
  const upcomingTasks = state.tasks.filter(task => {
    return task.status !== 'Done' && (task.due_date || task.take_action);
  });
  
  // Sort chronologically by due_date first, then take_action
